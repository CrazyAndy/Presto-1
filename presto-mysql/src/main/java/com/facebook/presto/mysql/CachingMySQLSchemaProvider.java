...
import static com.facebook.presto.mysql.RetryDriver.retry;
 ...
 */
@ThreadSafe
public class CachingMySQLSchemaProvider
{
    ...
    private final LoadingCache<PartitionListKey, List<MySQLPartition>> partitionsCache;
    private final LoadingCache<PartitionListKey, List<MySQLPartition>> partitionsCacheFull;
    private final LoadingCache<SchemaTableName, MySQLTable> tableCache;

    @Inject
    public CachingMySQLSchemaProvider(
            MYSQLConnectorId connectorId,
            MySQLSession session,
            @ForMySQLSchema ExecutorService executor,
            MySQLClientConfig mysqlClientConfig)
    {
        this(checkNotNull(connectorId, "connectorId is null").toString(),
                session,
                executor,
                checkNotNull(mysqlClientConfig, "mysqlClientConfig is null").getSchemaCacheTtl(),
                mysqlClientConfig.getSchemaRefreshInterval());
    }

    public CachingMySQLSchemaProvider(String connectorId, MySQLSession session, ExecutorService executor, Duration cacheTtl, Duration refreshInterval)
    {
        ...
        tableCache = CacheBuilder
                ...
                {
                    @Override
                    public MySQLTable load(SchemaTableName tableName)
                            throws Exception
                    {
        ...
        partitionsCache = CacheBuilder
                ...
                {
                    @Override
                    public List<MySQLPartition> load(PartitionListKey key)
                            throws Exception
                    {
    ...
    public MySQLTableHandle getTableHandle(SchemaTableName schemaTableName)
    {
        ...
        MySQLTableHandle tableHandle = new MySQLTableHandle(connectorId, schemaName, tableName);
        return tableHandle;
    }
    ...
    public MySQLTable getTable(MySQLTableHandle tableHandle)
            throws TableNotFoundException
    {
        return getCacheValue(tableCache, tableHandle.getSchemaTableName(), TableNotFoundException.class);
    }

    private MySQLTable loadTable(final SchemaTableName tableName)
            throws Exception
    {
        return retry().stopOn(NotFoundException.class).stopOnIllegalExceptions()
                .run("getTable", new Callable<MySQLTable>()
                {
                    @Override
                    public MySQLTable call()
                            throws TableNotFoundException
                    {
                        MySQLTable table = session.getTable(tableName);
                        return table;
                    }
                });
    }
    ...
    {
        LoadingCache<PartitionListKey, List<MySQLPartition>> cache;
    ...
    private List<MySQLPartition> loadPartitions(final PartitionListKey key)
            throws Exception
    {
        return retry().stopOnIllegalExceptions()
                .run("getPartitions", new Callable<List<MySQLPartition>>()
                {
                    @Override
                    public List<MySQLPartition> call()
                    {
    ...
    private static final class PartitionListKey
    {
        ...
        PartitionListKey(MySQLTable table, List<Comparable<?>> filterPrefix)
        {
            this.table = table;
