...
import com.facebook.presto.mysql.MySQLTableHandle;
...
public final class MySQLUtils
{
    ...
    private static void appendSelectColumns(StringBuilder stringBuilder, List<? extends ColumnHandle> columns, boolean first)
    {
        for (ColumnHandle column : columns) {
            ...
            stringBuilder.append(validColumnName(((MySQLColumnHandle) column).getName()));
        }
    }
    ...
    public static Selection select(List<MySQLColumnHandle> columns)
    {
        Selection selection = QueryBuilder.select();
        for (MySQLColumnHandle column : columns) {
            selection.column(validColumnName(column.getName()));
        }
        return selection;
    }

    public static Select selectFrom(MySQLTableHandle tableHandle, List<MySQLColumnHandle> columns)
    {
    ...
    public static Select selectDistinctFrom(MySQLTableHandle tableHandle, List<MySQLColumnHandle> columns)
    {
        return from(select(columns).distinct(), tableHandle);
    }

    public static Select selectCountAllFrom(MySQLTableHandle tableHandle)
    {
        String schema = validSchemaName(tableHandle.getSchemaName());
        String table = validTableName(tableHandle.getTableName());
